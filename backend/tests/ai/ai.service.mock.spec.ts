...
describe('AiService (modo mock)', () => {
  ...
  it('isEnabled es true en modo mock aunque no haya API key', () => {
    const service = new AiService(config, {} as PrismaService, {} as AiToolExecutorService, noMemory, noProfile, noKnowledge);
  ...
  it('ejecuta create_appointment cuando el mensaje menciona una cita', async () => {
    ...
    const service = new AiService(config, {} as PrismaService, tools, noMemory, noProfile, noKnowledge);
  ...
  it('recupera memoria de contexto también en modo mock (prueba la tubería completa sin gastar créditos)', async () => {
    ...
    const service = new AiService(config, {} as PrismaService, tools, contextMemory, noProfile, noKnowledge);
