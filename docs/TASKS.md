...
- [x] CRUD de contactos y bandeja de conversaciones (API). Módulos `contacts/` (`GET/POST /contacts`, `GET/PATCH /contacts/:id`) y `conversations/` (`GET /conversations` bandeja, `GET /conversations/:id` con hilo de mensajes, handoff/close/reopen). Todos los endpoints protegidos por JWT y filtrados por `tenantId` del token. Verificado end-to-end + tests de aislamiento por tenant. **Falta** el panel/UI mínimo (frontend). — agente: `dev-backend-api`.
