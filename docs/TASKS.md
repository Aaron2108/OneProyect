...
- [ ] Catálogo de productos con stock — pedido por el propietario junto con la subida de documentos, pero **no se resuelve con documentos**: un PDF es una foto de un momento y la IA prometería existencias que ya no hay. Necesita tabla `Product`, importación, y una herramienta `consultar_producto` (tool-calling contra la BD, como las citas), no inyección en el `system` prompt.
