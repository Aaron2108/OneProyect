...
**Capas de negocio dentro de `/src`** (a crear en Sprint 1, ver `TASKS.md`): `domain/` (entidades y reglas: contacto, conversación, cita, recordatorio), `application/` (casos de uso: recibir mensaje, generar respuesta, crear cita), `infrastructure/` (cliente de Meta Cloud API, cliente de Claude, repositorios PostgreSQL, colas), `presentation/` (controladores REST, webhook).
