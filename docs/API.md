...
| Google Calendar (Fase 3) | `GET /integrations/google-calendar/status`, `GET /integrations/google-calendar/connect-url`, `POST /integrations/google-calendar/check`, `POST /integrations/google-calendar/disconnect`, `GET /integrations/google-calendar/callback` | ✅ Conexión OAuth2 de un calendario de Google por tenant (solo OWNER conecta/desconecta); sincronización de una sola vía WhatsFlow→Google al crear/editar/cancelar una cita. `callback` es público (lo invoca el navegador desde Google); ver `SECURITY.md` §11. `check` fuerza un refresco del token contra Google y lee el calendario de destino: convierte "conectado" en algo verificado con fecha, y sube de paso las citas pendientes del tenant. |
